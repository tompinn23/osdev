...
buddy_t *buddy_init(int zone_ty, uint64_t base, uint64_t length) {
  ...
  for (int i = BUDDY_MAX_LEVEL - 1; i > 0; i--) {
    int64_t top_level_free = iDivCeil(length, ((1 << i) * PAGE_SIZE));
    if (top_level_free > 0) {
      for (int64_t i = 0; i < top_level_free; i++) {
        void *addr = get_ptr_for_index(bd, i, i - 1);
        list_push(&bd->free_area[i].free_list, addr);
      }
      break;
    }
  }
  for (int64_t i = 0; i < iDivCeil(map_length, PAGE_SIZE); i++) {
    __buddy_lock_page(bd, i);
  }
  return bd;
}
...
void *buddy_split_block(buddy_t *buddy, int order, int requested_order) {
  if (order == 0) /* You can't split a page dummy */
    return NULL;
  do {
    list_t *free = list_pop(&buddy->free_area[order].free_list);
    list_t *free_buddy = free + ((1 << (order - 1)) * PAGE_SIZE);
  ...
  if (free != NULL) {
    __change_bit(get_index_for_ptr(buddy, free, requested_order),
                 buddy->free_area[requested_order].map);
  }
  return free;
}
