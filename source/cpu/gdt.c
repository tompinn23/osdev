...
static __attribute__((aligned(0x1000))) gdt_desc_t gdt[GDT_MAX_DESCRIPTORS];
...
void gdt_assemble() {
  ...
  gdt_add_descriptor(0, 0, GDT_BASIC_DESCRIPTOR | GDT_DESCRIPTOR_EXECUTABLE,
                     GDT_BASIC_GRANULARITY);
  gdt_add_descriptor(0, 0, GDT_BASIC_DESCRIPTOR, GDT_BASIC_GRANULARITY);
  gdt_add_descriptor(0, 0, GDT_BASIC_DESCRIPTOR | GDT_DESCRIPTOR_DPL,
                     GDT_BASIC_GRANULARITY);
...
void gdt_add_descriptor(uint64_t base, uint16_t limit, uint8_t access,
                        uint8_t granularity) {
